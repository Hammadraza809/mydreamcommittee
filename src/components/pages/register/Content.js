...
export default function Main(props) {
  ...
  useEffect(() => {
    async function getData() {
      const res = await fetch(
        `https://mydreamcommittee.com/v1/controller/committee.php?value=${commt}`
      );
  ...
  const onRegister = (data) => {
    ...
    setLoading(true);
    fetch(`https://mydreamcommittee.com/v1/controller/user.php`, {
      method: "POST",
      headers: {
      ...
      .then((result) => {
        ...
        fetch(
          `https://mydreamcommittee.com/v1/controller/image.php?memberid=${id}`,
          {
            method: "POST",
            body: formdata,
          }
